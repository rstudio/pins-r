...
```{r}
retail_sales %>%
  group_by(month = month(ds, T)) %>%
  summarise(total = sum(y)) %>%
  pin("sales_by_month")
```

You can also find remote files using `pin_find()` which can find files embedded in CRAN packages, but you can also configure Kaggle with ease with `board_register()`. For instance, we can search resources mentioning "seattle":
...
Or load the resource contents through `fetch()`,
...
pd.read_csv(pins.fetch("hpiR/seattle_sales"))
```
