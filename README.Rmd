...
You can also use pins to cache intermediate results to avoid having to recompute expensive operations. The following example stores results computed with `dplyr` into a `sales_by_month`, which you can then retrive with `pin_get()` even after your R session restarts:
...
You can use [RStudio](https://www.rstudio.com/products/rstudio/) and [RStudio Connect](https://www.rstudio.com/products/connect/) to discover and share resources within your organization with ease.

To enable new boards, you can use [RStudio's Data Connections](https://blog.rstudio.com/2017/08/16/rstudio-preview-connections/) to start a new 'pins' connection and then select which board to connect to:
