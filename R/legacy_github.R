...
legacy_github <- function(
  ...
  # check repo exists
  check_exists <- httr::GET(
    github_url(board, branch = NULL),
  ...
  if (httr::http_error(check_exists)) {
    stop(
      ...
      httr::content(check_exists, encoding = "UTF-8")$message
    )
  }
  ...
  if (
    "error" %in%
      class(branches) &&
      grepl("^Git Repository is empty", branches$message)
  ) {
    github_update_index(
      board,
      "",
  ...
  if (is.null(branch)) {
    if ("main" %in% branches) {
      ...
      abort(
        "No 'master' or 'main' branch present; please use `branch` argument"
      )
    }
...
#' @export
board_register_github <- function(
  name = "github",
  repo = NULL,
  branch = NULL,
  token = NULL,
...
github_update_temp_index <- function(
  board,
  path,
  commit,
  operation,
  ...
  if (!httr::http_error(response)) {
    ...
    if (!is.null(content$content)) {
      index <- board_manifest_load(rawToChar(jsonlite::base64_dec(
        content$content
      )))
    } else {
      response <- httr::GET(content$download_url, github_headers(board))
      if (!httr::http_error(response)) {
        index <- board_manifest_load(httr::content(
          response,
          encoding = "UTF-8"
        ))
      }
...
github_update_index <- function(
  ...
  index <- github_update_temp_index(
    board,
    path,
    commit,
    operation,
...
github_delete_release <- function(board, release_tag) {
  response <- httr::GET(
    github_url(board, branch = NULL, paste0("/releases/tags/", release_tag)),
    github_headers(board)
  )
  if (!httr::http_error(response)) {
    release_id <- httr::content(response, encoding = "UTF-8")$id
    response <- httr::DELETE(
      github_url(board, branch = NULL, paste0("/releases/", release_id)),
      github_headers(board)
    )
    if (httr::http_error(response)) {
    ...
    response <- httr::DELETE(
      github_url(board, branch = NULL, paste0("/git/refs/tags/", release_tag)),
      github_headers(board)
    )
    if (httr::http_error(response)) {
...
github_create_release <- function(board, name) {
  ...
  release <- list(
    ...
    body = paste0(
      ...
      name,
      "' which is too large to be stored as a GitHub file."
    )
  )
...
github_upload_content <- function(
  board,
  name,
  file,
  file_path,
...
github_files_commit <- function(board, upload_files, branch, commit) {
  ...
  tree_result <- github_create_tree(
    board,
    unname(tree_files),
    head_sha$object$sha
  )
  commit_result <- github_create_commit(
    board,
    tree_result$sha,
    head_sha$object$sha,
    commit
...
#' @export
board_pin_create.pins_board_github <- function(
  board,
  path,
  name,
  metadata,
  ...
  for (file in upload_files) {
    ...
    if (
      (file.info(file_path)$size >
        getOption("pins.github.release", 25) * 10^6 ||
        release_storage) &&
...
board_pin_find.pins_board_github <- function(board, text, ...) {
  ...
  if (!httr::http_error(result)) {
    ...
    pin_log(
      "Failed to find 'data.txt' file in repo '",
      board$repo,
      "', path '",
      board$path,
      "' and branch '",
  ...
  if (is.character(text)) {
    result <- result[
      grepl(text, result$name) | grepl(text, result$description),
    ]
  }
  ...
  if (nrow(result) == 1) {
    ...
    if (!httr::http_error(result_single)) {
      local_path <- pin_download_files(
        httr::content(result_single, encoding = "UTF-8")$download_url,
        result$name,
        board,
      ...
      result$metadata <- as.character(jsonlite::toJSON(
        manifest,
        auto_unbox = TRUE
      ))
    }
...
github_branch <- function(board, branch) {
  ...
  if (httr::http_error(response)) {
    stop(
      ...
      as.character(httr::content(response, encoding = "UTF-8"))
    )
  }

  branch_object <- Filter(
    function(e) identical(e$ref, reference),
    httr::content(response, encoding = "UTF-8")
  )
...
github_content_url <- function(board, branch = board$branch, ...) {
  ...
  url <- paste0(
    ...
    board$repo,
    "/contents/",
    paste0(board$path, paste0(args, collapse = ""))
  )
  if (!is.null(branch)) {
    url <- paste0(url, "?ref=", branch)
  }
...
#' @export
board_pin_get.pins_board_github <- function(
  board,
  name,
  extract = NULL,
  version = NULL,
...
board_pin_remove.pins_board_github <- function(board, name, ...) {
  ...
  result <- httr::GET(
    paste0(base_url, "?ref=", board$branch),
    github_headers(board)
  )
