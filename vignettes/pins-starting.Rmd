...
You can discover datasets with `pin_find()`, which by default will search for data inside CRAN packages. The places where `pins` can find or store resources are referred as 'boards'. There are multiple boards available but they require you to configure them so we will leave those for later on.
...
To be fair, storing pins locally is not really sharing with others; instead, you would want to replace `local` with `github`, `rsconnect` or `kaggle`. However, we will first have to configure the appropriate boards, this will be presented in the [Understanding Boards](boards-github.html) article. Before we get to that, the [Using Pins with RStudio](pins-rstudio.html) article will present how to use the functionality presented here using RStudio's user interface.
