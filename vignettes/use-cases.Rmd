...
There are many interesting use cases available for `pins`, from easily caching remote resources to creating complex data pipelines which we can't possible describe exhaustively. Therefore, the purpose of this section is to mention a few use cases that can help you get inspired on how to use `pins`.

For instance, after cleaning up your data, you can consider letting others [reuse your tidy datasets](use-cases/reusing-tidy-datasets.html). Once your data is in a shared location, you might want to consider [automating dataset updates](use-cases/automate-dataset-updates.html). You can then attempt to pipe multiple datasets to [create simple data pipelines](use-cases/create-simple-data-pipelines.html) or partially [update plumber and shiny apps](use-cases/update-plumber-and-shiny-apps.html) using pins.
...
pins_site_gallery(list(
  list(image       = "images/use-cases-reuse-tidy-datasets.png",
   ...
   list(image      = "images/use-cases-create-simple-pipelines.png",
       title       = "Create Simple Pipelines",
       href        = "use-cases/create-simple-pipelines.html",
