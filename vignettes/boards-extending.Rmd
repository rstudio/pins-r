...
The next set of S3 methods to consider implementing are `board_initialize()`, `board_load()` and `board_persist()`. There methods are optional to implement but provide support for initialization, loading a board from disk and persisting a board to disk. The initialization method usually deals with authentication, while the load and persist methods can be used to avoid storing secrets or other temporal parameters to disk.
...
The following example creates a simple folder-based board; probably not suitable for real use cases since all your pins would be lost whenever the R session restarts; however, it's a simple example that you can use to start developing custom boards:
