...
The pins package provides a robust set of methods to read and write standard types of files using standard tools, e.g. CSV files using `read.csv()` and `write.csv()`.
However, from time to time, you may wish read or write using other tools. Perhaps you want to read and write:

 - CSV files using readr or vroom.
 - Arrow files without using compression. 
 - Parquet files.

An escape hatch is provided: `pin_upload()` and `pin_download()`. 
The goal of this vignette is to show how you can incorporate these into your workflow.

Two points to keep in mind:

 - a vector of `paths` to files is provided to `pin_upload()`.
 - a vector of `paths` to files is returned from `pin_download()`.

Let's follow the "uncompressed Arrow" example; we'll start by creating a temporary board:
...
# file name will be `mtcars-vroom.arrow`
...
If you want to write more than one custom file of a certain type, like uncompressed Arrow, you might consider writing a helper function:
