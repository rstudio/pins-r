...
You can then search pins in your repo using `pin_find()`, which by default search all boards but you can also explicitly request to use this particular RStudio Connect board:
...
Once you share with specific users, they can follow the same steps to register their RStudio Connect board which allows them to download pins shared with them or upload their own pins.

## Automation

One significant advantage from using RStudio Connect over other boards is it's ability to schedule R Markdown reports to run automatically. This allows you automate the creation of pins and also consume pins published manually from automated reports.
...
However, since it's a best practice to not store secrets in plain text, `pins` will read also the key from the `RSCONNECT_API` environment variable, or you can retrieve explicitly as `Sys.getenv("RSCONNECT_API")` before assigning it to the key parameter.
...
library(pins)
register_board_rstudio(key = Sys.getenv("RSCONNECT_API"), server = "https://rstudio-connect-server")
```
...
The RStudio Connect board also supports RStudio Connect [Output Files](https://docs.rstudio.com/connect/user/r-markdown.html#r-markdown-output-files) by setting `output_files = TRUE` when registering an automated report.
