...
In `pins 0.3`, when you use a board that starts with `https://`, `pins` will automatically register this board as a [website board](https://rstudio.github.io/pins/articles/boards-websites.html) to save you from having to explicitly call `board_register_datatxt(url = "https://rpins.s3.amazonaws.com", name = "rpins")`.
...
Boards likee Kaggle and RStudio Connect, store additional information for each pin which you can now esily retrieve with `pin_info()`.

For instance, we can retrieve additional informationfrom the `worldbank/indicators` directly from Kaggle as follows,

```r
pin_info("worldbank/world-development-indicators")
```
```
...
Thank for reading this post! Please use [rstudio.github.io/pins](https://rstudio.github.io/pins) for additional news and documentation, [GitHub](https://github.com/rstudio/pins/issues/new) to open issues we should investigate and [Gitter](https://gitter.im/rstudio/pins) to chat with us.
