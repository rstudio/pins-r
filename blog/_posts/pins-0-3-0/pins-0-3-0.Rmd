...
- Use new **cloud boards** to pin resources in [Azure](../../../articles/boards-azure.html), [GCloud](../../../articles/boards-gcloud.html) and [S3](../../../articles/boards-s3.html) storage.
- Retrieve **pin information** with `pin_info()` including metadata particular to each board.
...
The `Indicators.csv` file is interesting, so let's load it with [readr](https://readr.tidyverse.org/):
...
Analysing the indicators dataset is quite interesting; however, this focuses on how to share this in S3, Google Cloud or Azure storage. More specifically, we will learn how to publish to an [S3 board](../../../articles/boards-s3.html). To publish to other cloud providers, take a look at the [Google Cloud](../../../articles/boards-gcloud.html) and [Azure boards](../../../articles/boards-azure.html) articles.

As usual, first we need to register the S3 board. When using RStudio, you can use the "New Connection" action to guide you through this process, or you can specify your `key` and `secret` as follows. Please refer to the [S3 board](../../../articles/boards-s3.html) article to understand how to store your credentials securely.
...
That's about it! We can now find and retrieve this dataset from S3 using `pin_find()` and `pin_get()`.
...
This works since `pins 0.3` automatically register URLs as a [website boards](../../../articles/boards-websites.html) to save you from having to explicitly call `board_register_datatxt()`.
...
This `pins` makes it a great tool for easily sharing datasets in your organization or the world.
...
For instance, we can retrieve additional information from the indicators pin from Kaggle as follows,
...
Or from RStudio Connect boards,
...
Please refer to [rstudio.github.io/pins](https://rstudio.github.io/pins) for detailed documentation, open a [new GitHub](https://github.com/rstudio/pins/issues/new) issues for use-cases we should investigate and [Gitter](https://gitter.im/rstudio/pins) to chat with us about anything else.
