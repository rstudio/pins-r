...
We can then easily download the "World Development Indicators", beware this is a 2GB download, good thing `pins 0.3` adds a progress bar!

```
pin_get("worldbank/world-development-indicators", board = "kaggle")
```
```
...
```r
indicators <- pin_get("worldbank/world-development-indicators", board = "kaggle")[6] %>%
  readr::read_csv()
```
...
To make this even easier for others to consume, we've made this S3 bucket public; which means you can now connect to the `rstudio-datasets` board without even having to configure S3! Since we've made the `rstudio-datasets` bucket available under the `datasets.rstudio.com` domain, you can easily connect to this board and retrieve this datasets with ease:

```r
library(pins)
board_register_datatxt(name = "rstudio-datasets", url = "datasets.rstudio.com)

pin_get("worldbank/indicators")
```

It's worth mentioning that `pins` stores the dataset using an R native format, which requires only 72MB and loads much faster than the original CSV and the original 2GB dataset!
